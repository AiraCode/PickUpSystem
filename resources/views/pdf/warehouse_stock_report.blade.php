...
<body>
    ...
    <table class="header-table">
        <tr>
            ...
            <td class="doc-title" style="width: 55%;">
                ...
                <div class="status-badge">SIAP DIAMBIL PUSAT (≥ 20 AKI)</div>
            </td>
        </tr>
    </table>
        ...
        <tr>
            <td>
                ...
                <div class="meta-item">
                    ...
                    <div class="meta-value">{{ isset($warehouse->city) && $warehouse->city ? $warehouse->city->name : 'Gudang Cabang ' . $warehouse->name }}</div>
                </div>
                <div class="meta-item">
                    <div class="meta-label">Alamat Lengkap Gudang</div>
                    <div class="meta-value" style="font-weight:400; font-size:10px;">{{ $warehouse->address ?: '-' }}</div>
                </div>
            </td>
            <td style="border-left: 1px solid #e2e8f0;">
                ...
                <div class="meta-item">
                    ...
                    <div class="meta-value">{{ $adminName }} ({{ $adminEmail }})</div>
                </div>
                <div class="meta-item">
                    <div class="meta-label">Total Akumulasi Aki Siap Diambil</div>
                    <div class="meta-value" style="color:#2563eb; font-size:12px;">{{ number_format($totalQty, 0, ',', '.') }} Unit</div>
                </div>
            </td>
        </tr>
    </table>
        ...
        <tbody>
            @forelse($items as $index => $item)
                ...
                <tr>
                    ...
                    <td class="text-center" style="font-weight:700; color:#1d4ed8;">{{ number_format($qty, 0, ',', '.') }} Pcs</td>
                    <td class="text-right" style="font-weight:600;">{{ number_format($subtotalWeight, 2, ',', '.') }} kg</td>
                </tr>
            @empty
                <tr>
            ...
            <tr class="grand-total-row">
                <td colspan="4" class="text-right">GRAND TOTAL STOK SIAP DIAMBIL:</td>
                <td class="text-center" style="font-size:12px;">{{ number_format($totalQty, 0, ',', '.') }} Pcs</td>
                <td class="text-right" style="font-size:12px;">{{ number_format($totalWeight, 2, ',', '.') }} kg</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="notice-box">
        <p><strong>Catatan Sistem:</strong> Dokumen struk ini diterbitkan secara otomatis oleh sistem saat jumlah stok aki di gudang mencapai ambang batas (≥ 20 unit). Mohon dilakukan verifikasi fisik jumlah unit dan jenis aki saat proses serah terima barang ke Kurir Pusat.</p>
    </div>
    ...
    <div class="footer-note">
        Struk Resmi Laporan Stok Aki Gudang &bull; Printed via PickUpSystem &bull; {{ date('d/m/Y H:i:s') }} WIB
    </div>
